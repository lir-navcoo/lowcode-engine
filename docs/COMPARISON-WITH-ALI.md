...
| `packages/workspace` (13 files, 1,298 lines) | Multi-window / multi-view support. | ✅ Shipped as `@monbolc/lowcode-workspace` (L5) at v2.1.2, 3 src files (~280 lines), 24 tests. **Decision 2026-06-07**: sapu keeps the *type* surface (`Resource`, `EditorWindow`, `Workspace` classes) for ali-port plugin compatibility, but drops the multi-window `Workbench` tab UI, the `ResourceType` registry, and the `openEditorWindowByResource` machinery. Multi-document = multi-mount of `<Skeleton>`. ~78% smaller. See `docs/packages/workspace.md` + `ROADMAP.md` P3 → L5. Demo's "Open second doc" button proves the multi-mount stance. |
