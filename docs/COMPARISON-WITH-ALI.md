...
## User-perceived gap matrix (2026-06-09)

| Area | AliLowcodeEngine has | Sapu currently has | User-perceived difference | Priority |
|---|---|---|---|---|
| Workbench / Dock / multi-area shell | Workbench, Stage, PanelDock, Dock, DialogDock, toolbar/left/right/bottom plugin areas | Lightweight 3-pane Skeleton with `topArea`, `leftArea`, `designerView` slots; no full Dock/Workbench system | Ali feels like a full IDE; sapu feels like a focused editor with fewer plugin extension entry points | P0 |
| Material panel | Pluginized left-side material/block/data/js panels | `MaterialPane` with categories + search, but not Dock-based | Component picking works, but users do not get Ali-style left toolbar panel switching | P1 |
| Settings / setter experience | Full setting field abstraction, dynamic setters, expression/variable/event popups | `SettingsPanel` + `SettingsPrimaryPane`; BaseUI setters; no complete `createField` / expression / event binding flow | Simple props are editable; complex material configuration feels thinner than Ali | P0 |
| Outline tree | Search/filter, drag, visibility, lock, rename, delete, condition/loop indicators | `react-arborist` outline with selection/expand/rename/delete pieces; row actions and indicators not fully Ali-level | Users can navigate the tree, but cannot manage complex structure as efficiently as in Ali | P0 |
| Canvas simulator isolation | iframe simulator, environment/resource consumer, asset loading | Single-DOM lightweight simulator/canvas | Complex pages may suffer style/environment leakage; preview feels less like a real sandbox | P0 |
| Canvas editing | Mature hover/select/BemTools/insertion/resize/live editing/scroll actions | Dragon, selection, insertion, context menu, BemTools pieces, clipboard/scroller/detecting pieces | Basic drag/select works; resize/live text editing/advanced context actions still feel less complete | P0 |
| Runtime schema execution | BaseRenderer context, JSExpression, DataSource, lifecycle, richer fault handling | Slim renderer with component mapping and React 19 renderers; JSExpression/DataSource runtime is still incomplete | Static/simple schemas render; data-bound or expression-heavy schemas do not behave like Ali | P0 |
| Asset / icon pipeline | Asset loading and icon font pipeline | Phase E still pending; some icon placeholders remain | Toolbar/material visual feedback looks less polished | P1 |
| Plugin API compatibility | Large `IPublicApi*` / `IPublicModel*` proxy surface, `commonUI`, plugin exports/preferences/registerLevel | Plain plugin context with real references; no Fusion CommonUI; no full proxy zoo | Native sapu plugins are straightforward, but Ali plugin/tutorial code cannot be copied directly | P0 |
| Plugin collaboration | Async init, dependencies/meta, preferences, exports between plugins | Lightweight plugin registration and lifecycle | Simple plugins work; complex plugin ecosystems miss config/dependency/export conventions | P1 |
| Multi-document workflow | Workspace/workbench multi-view UX | Multi-mount Skeleton stance; workspace model is slim | Users do not see Ali-style tabbed multi-document workbench | P2 |
| React / UI compatibility | React 16 + Fusion + legacy DOM patterns | React 19 + BaseUI + no `findDOMNode` | Modern and cleaner, but old Ali/Fusion plugins require migration | Strategy difference |

**Recommended user-value order:** first close the visible editing gaps (setter/expression/event config, DataSource/JSExpression runtime, outline row actions, context menu/placement), then add platform-shell capabilities (Area/Dock/Popup/Hotkey/EventBus), then consider Ali plugin migration facades.
...
|---|---|---|
| `packages/engine` (15 files, 1,330 lines) | Composition root: `init()`/`destroy()`, registers all inner plugins, UMD bundles. | ✅ Shipped as `@monbolc/lowcode-engine` (L7) at v2.2.0, 5 src files (~310 lines), 18 tests. **Decision 2026-06-08**: sapu drops the UMD bundler config (no `dist/engine.js`), drops the auto-registration of 7 inner plugins (sapu ships 3: outline-pane, settings-panel, setters), drops the async init + topo sort, drops the `window.aliLowcodeEngine` global, drops the `setAsInstance`/`current` machinery. Hosts call `init(container, { schema, components, preset })` and get back the live `SapuEngine`. ~77% smaller. See `docs/packages/engine.md` for the full table. |
| `packages/shell` (45 files, 5,155 lines) | The `IPublicApi*` / `IPublicModel*` facade layer. Re-exports 28 `@alifd/next` components as `IPublicApiCommonUI`. | ✅ Shipped as `@monbolc/lowcode-shell` (L6) at v2.2.0, 4 src files (~530 lines), 21 tests. **Decision 2026-06-08**: sapu drops the 28-component re-export (Fusion removed), drops the per-plugin scoped bus + deprecation proxies, drops async `init` + topo sort. Plugins get a plain `IPluginContext` with real references. The class is React-free (`SapuErrorBoundary` is a separate React 19 component). ~90% smaller. See `docs/packages/shell.md` for the full table. |
| `packages/workspace` (13 files, 1,298 lines) | Multi-window / multi-view support. | ✅ Shipped as `@monbolc/lowcode-workspace` (L5) at v2.2.0, 3 src files (~280 lines), 24 tests. **Decision 2026-06-07**: sapu keeps the *type* surface (`Resource`, `EditorWindow`, `Workspace` classes) for ali-port plugin compatibility, but drops the multi-window `Workbench` tab UI, the `ResourceType` registry, and the `openEditorWindowByResource` machinery. Multi-document = multi-mount of `<Skeleton>`. ~78% smaller. See `docs/packages/workspace.md` + `ROADMAP.md` P3 → L5. Demo's "Open second doc" button proves the multi-mount stance. |
| `packages/plugin-designer` (1 file, 157 lines) | The "designer" widget glue file (renders `<DesignerView>` in mainArea). | ✅ Shipped as L4 `Skeleton.designerView` prop (P2.4, 2026-06-08). Sapu 不单独发包, 把 157 行的 `DesignerView` 吸收成 `<DefaultDesignerView>` 组件 (~120 行), 通过 `designerView?: (helpers) => ReactNode` prop 让 host 完整替换. 上游的 `DesignerProps.simulatorComponent` 插槽对应 sapu 的 `designerView(helpers)`. |
...
- Manual `useEffect` + `emitter.on/off` subscriptions inside components that need to re-render (no `useRev` counter shim — every subscription targets specific event names so re-renders stay minimal)
...
All upstream `IPublic*` types that belong to `shell/api` (cabin), `shell/model` (facade), legacy `shell/type`, `deprecated/`, Fusion theme/CommonUI, assets JSON, and old global config remain intentionally out of L0. These are either React/Fusion-era compatibility surfaces or Ali proxy zoo surfaces, not current sapu public API.

As of 2.5.0, sapu has re-introduced the authoring-critical slim ports in `@monbolc/lowcode-types`: location / setting / presentational / workspace / drag (2.4.0) plus component metadata, JS expression/function/block/slot, prop config, dynamic setter, plugin declaration, transducer, rich project schema, action content object, editor view, and `IPublicEnum*` names (2.5.0). See `docs/packages/types.md` for the exact list.

Still-deferred examples: `IPublicApiCommonDesignerCabin`, `IPublicApiCommonEditorCabin`, `IPublicApiCommonSkeletonCabin`, `IPublicApiCommonUtils`, `IPublicModelSettingTopEntry`, `IPublicModelSettingPropEntry`, `IPublicModelEditorView`, `IPublicModelEngineConfig`, `IPublicModelSimulatorRender`, `IPublicModelClipboard`, `IPublicTypePanelDockConfig`, `IPublicTypeWidgetConfigArea`, `IPublicTypeWidgetBaseConfig`, `IPublicTypeLiveTextEditingConfig`, `IPublicTypeContextMenuAction`, `IPublicTypeAssetsJson`, `AssetType`, `AssetLevel`, `FusionTheme`, and the entire `deprecated/` folder.

Sapu can add more names back only when a real package or plugin needs them. The rule remains: authoring types are okay; shell/model proxy compatibility is not automatically restored.
