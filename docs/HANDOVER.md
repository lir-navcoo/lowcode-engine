...
- **当前分支**: `main` @ `492bbfd` (干净，已推 origin)
- **下次发布**: **2.2.0**，代码 + 测试 + 构建全部就绪，**但 npm publish 阻塞**（见下）
- **测试**: 951 unit passed + 1 skipped (vitest 2.1, happy-dom) + 11 e2e (Playwright 1.60)
...
- **包版本**: 14 个 `@monbolc/*` 包 — designer 2.27.0 (D + D.I7b.1-13), editor-skeleton 2.4.0 (D.I7b.4), types 2.3.0 (D.I2), others 2.2.0
...
- **ali-mirror 计划**: Phase A + B + C.X + C.Y + C.Z + C.AA + C.AB + C.AC + C.AD + C.AE + D (10 commits, 5000+ LoC) + D.I7b.1-13 (InsertionView real + NodeSelector BaseUI Popover + BorderResizing real + BemTools wired + BaseUI Tooltip + context menu + valuechange event + live-editing Escape/Enter keys + resource-consumer throw + error reporting) 全部完成。**Phase D + D.I7b.1-13 收官**。Phase E (asset pipeline + icon font + iframe-mode simulator) 仍待办。详见 `~/.claude/plans/dynamic-marinating-rabbit.md` 和 `docs/ROADMAP.md`。
