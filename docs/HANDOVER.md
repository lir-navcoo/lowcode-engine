...
- **当前分支**: `main` @ `5e137f8` (干净，已推 origin)
- **下次发布**: **2.2.0**，代码 + 测试 + 构建全部就绪，**但 npm publish 阻塞**（见下）
- **测试**: 644 unit passed + 1 skipped (vitest 2.1, happy-dom) + 11 e2e (Playwright 1.60)
...
- **ali-mirror 计划**: Phase A + B + C.X + C.Y + C.Z 已完成。Phase C 剩余(dragon HTML5 DnD / sensor array / autorun shims / OffsetObserver auto-subscribe) + Phase D (simulator + bem-tools) 待办。详见 `~/.claude/plans/dynamic-marinating-rabbit.md` 和 `docs/ROADMAP.md`。
